...
const SettingsTodo = () => {
    ...
    function updateProducts() {
        let products = localStorage.getItem('todos') || '[]';
    ...
    function deleteProduct(pid: number, did: string) {
        let products = JSON.parse(localStorage.getItem('todotable') || "[]")
        if(products) {
            products[did] = products[did].filter((item2: Product) => {
                return item2.id !== pid
            })
            localStorage.setItem('todotable', JSON.stringify(products))
        }
        updateProducts()
        updateHavtable()
    }
    ...
    function addProduct(item: Product) {
        ...
        let data = JSON.parse(localStorage.getItem('todotable') || "[]")
